...
const PrinterCard = ({ printer, onViewDetails, onStartMaintenance, isDragging = false }: PrinterCardProps) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  // Get status color for the top border
  ...
  const temperatures = getMockTemperatures();
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium capitalize">{printer.status}</span>
            </div>
...
const PrinterRow = ({ printer, onViewDetails, onStartMaintenance }: PrinterRowProps) => {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    : printers.filter(printer => {
        switch (statusFilter) {
          ...
          case 'needs-attention':
            return printer.status === 'maintenance';
          default:
            return true;
        }
      });
