...
import { discoverPrinters, getHubStatus, isHubOnline } from "../lib/hub-commands";
...
// =============================================================================
// HUB SELF-REGISTRATION (Public endpoint for ESP32)
// =============================================================================
 ...
 * Public endpoint for ESP32 hubs to self-register
 *
 * Flow:
 * 1. Hub boots up with generated ID (HUB-{MAC})
 * 2. Hub calls this endpoint to register itself
 * 3. If tenant_id and claim_token are provided, validates and claims the hub
 * 4. Returns success so hub can connect via WebSocket
 */
hubs.post("/register", async (c) => {
  // Parse and validate request body
  ...
  if (existingHub) {
    // Hub already registered - check if trying to claim with different tenant
    if (body.tenant_id && body.claim_token) {
      if (existingHub.tenant_id && existingHub.tenant_id !== body.tenant_id) {
        throw new ApiError(
          "Hub is already claimed by another tenant",
          409,
          "ALREADY_CLAIMED"
        );
      }

      // Trying to claim an unclaimed hub or re-claim for same tenant
      if (!existingHub.tenant_id) {
        // Validate claim token
        const tenant = await c.env.DB.prepare(
          "SELECT id, hub_claim_token FROM tenants WHERE id = ?"
        )
          .bind(body.tenant_id)
          .first<{ id: string; hub_claim_token: string | null }>();

        if (!tenant) {
          throw new ApiError("Tenant not found", 404, "TENANT_NOT_FOUND");
        }

        if (!tenant.hub_claim_token || tenant.hub_claim_token !== body.claim_token) {
          throw new ApiError("Invalid claim token", 401, "INVALID_CLAIM_TOKEN");
        }
        ...
        await c.env.DB.prepare(
          ...
          .bind(body.tenant_id, `Hub ${body.hub_id.slice(-6)}`, now, now, body.hub_id)
          .run();

        return c.json({
          success: true,
          message: "Hub registered and claimed",
          data: {
            hub_id: body.hub_id,
    ...
    // Hub already exists, just update metadata
    await c.env.DB.prepare(
      `UPDATE hubs SET
      ...
      .bind(body.mac_address || null, body.firmware_version || null, now, body.hub_id)
      .run();

    return c.json({
      success: true,
      message: existingHub.tenant_id ? "Hub already registered and claimed" : "Hub already registered",
      data: {
        hub_id: body.hub_id,
        tenant_id: existingHub.tenant_id,
        claimed: !!existingHub.tenant_id,
      },
    });
  }

  // Create new hub entry
  // Generate a random secret for the hub (used for future HMAC authentication)
  const secretBytes = new Uint8Array(32);
  crypto.getRandomValues(secretBytes);
  const secretHash = await sha256(Array.from(secretBytes).map(b => b.toString(16).padStart(2, '0')).join(''));

  let tenantId: string | null = null;
  let claimed = false;

  // If claim info provided, validate and claim
  if (body.tenant_id && body.claim_token) {
    const tenant = await c.env.DB.prepare(
      "SELECT id, hub_claim_token FROM tenants WHERE id = ?"
    )
    ...
    if (tenant && tenant.hub_claim_token === body.claim_token) {
  ...
  await c.env.DB.prepare(
    `INSERT INTO hubs (id, tenant_id, name, secret_hash, mac_address, firmware_version, claimed_at, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  )
    .bind(
      body.hub_id,
      tenantId,
      claimed ? `Hub ${body.hub_id.slice(-6)}` : null,
      secretHash,
  ...
  return c.json(
    {
      success: true,
      message: claimed ? "Hub registered and claimed" : "Hub registered",
      data: {
        hub_id: body.hub_id,
        tenant_id: tenantId,
        claimed: claimed,
